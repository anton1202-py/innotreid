...
class WildberriesFbsMode():
    ...
    def create_delivery(self):
        ...
        try:
            delivery_date_wb = datetime.today().strftime("%d.%m.%Y %H-%M-%S")
            url_data = 'https://suppliers-api.wildberries.ru/api/v3/supplies'
            hour = datetime.now().hour
            delivery_name = f"Поставка {delivery_date_wb}"
            if hour > 6 and hour < 18:
                delivery_name = f'День {delivery_date_wb}'
            else:
                delivery_name = f'Ночь {delivery_date_wb}'
...
class YandexMarketFbsMode():
    ...
    def saving_tickets(self):
        ...
        try:
            ...
            folder_summary_file_name = f'{raw_ticket_folder}/YANDEX - ИП этикетки {self.date_for_files}.pdf'
            merge_barcode_for_yandex_two_on_two(
    ...
    def create_yandex_selection_sheet_pdf(self):
        ...
        try:
            ...
            folder = (
                f'{self.dropbox_current_assembling_folder}/YANDEX - ИП лист подбора {self.date_for_files}.pdf')
            # Сохраняем на DROPBOX
            with open(output, 'rb') as f:
                dbx_db.files_upload(f.read(), folder)
        except Exception as e:
    ...
    def create_pivot_xls(self):
        ...
        try:
            ...
            for key, value in sorted_data_for_pivot_xls.items():
                ...
                COUNT_HELPER += 1
            folder_path_excel = os.path.join(
