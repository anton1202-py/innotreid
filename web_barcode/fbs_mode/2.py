...
class OzonFbsMode():
    ...
    def __init__(self, ozon_headers, db_folder, file_add_name):
        ...
        self.date_before = datetime.now() - timedelta(days=30)
...
class CreatePivotFile(WildberriesFbsMode, OzonFbsMode, YandexMarketFbsMode):
    ...
    def create_pivot_xls(self):
        ...
        print('self.amount_articles в самом начале', self.amount_articles)
        hour = datetime.now().hour
        date_folder = datetime.today().strftime('%Y-%m-%d')
        if hour >= 6 and hour < 18:
            self.delivary_method_id = self.warehouse_dict['day_stock']
        ...
        print('self.amount_articles после добавления ОЗОН', self.amount_articles)
        ...
        print('self.amount_articles после добавления яндекса', self.amount_articles)
    ...
    def analyze_fbs_amount(self):
        ...
        if self.wb_article_amount:
            print('self.wb_article_amount', self.wb_article_amount)
...
production_file(db_folder, file_add_name_ip, wb_headers_karavaev,
